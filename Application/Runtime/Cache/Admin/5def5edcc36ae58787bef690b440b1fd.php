...
<div class="page-container">
	<form onSubmit="return false;" class="form form-horizontal" id="form-article-add">
		<div class="row cl">
            ...
            <input type="hidden" name="delimg" value="<?php echo ($image); ?>" />
			<label class="form-label col-xs-4 col-sm-2">产品标题：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" placeholder="产品标题" value="<?php echo ($title); ?>" name="title">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">产品简介：</label>
			<div class="formControls col-xs-8 col-sm-9">
                <textarea class="textarea" name="content" id="content" placeholder="产品简介"><?php echo ($content); ?></textarea>
			</div>
		</div>
		<div class="row cl">
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<button onClick="subimg();" class="btn btn-primary radius" type="submit"><i class="Hui-iconfont">&#xe632;</i> 保存 </button>
			</div>
		</div>
	</form>
</div>
...
function subimg(){
    ...
    var delimg = $("input[name='delimg']").val();
    if(title == ''){
    	layer.msg('您需要填写标题');
    	return false;
    }
    ...
    $.ajax({
        url:"<?php echo U('Index/productCenterUpda');?>",
        type:'post',
        dataType:'json',
        data:{'image':image,'title':title,'content':content,'id':id,'delimg':delimg},
        success: function(data){
            if(data.msg == 1){
                layer.msg('保存成功');
                window.location.href="<?php echo U('Index/productCenter');?>";
            }else if(data.msg == 2){
                layer.msg('您需要先上传图片');
            }else{
                layer.msg('您没有修改任何内容');
